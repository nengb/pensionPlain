...
// 玩家排名
exports.get_user_ranking = function (start, row, callback) {
    callback = callback == null ? nop : callback;

    var sql1 = 'SELECT count(*) as cnt FROM t_users';
    query(sql1, function (err, rows1, fields) {
        if (err) {
            callback(null);
            return console.error(err.stack);
        }
        var sql = 'SELECT * FROM t_users order by (total_games+total_score) desc ,total_games asc,userid asc limit '+start+','+row;
        ...
        query(sql, function (err, rows, fields) {
            ...
            for (var i = 0; i < rows.length; i++) {
                j.push({
                    ...
                    name: crypto.fromBase64(rows[i].name),
                    score: rows[i].total_games + rows[i].total_score,
                    rank: parseInt(start) + i + 1,
                    total_games:rows[i].total_games,
...
// 房间总数
exports.get_rooms = function (start, row, callback) {
    callback = callback == null ? nop : callback;

    var sql1 = 'SELECT count(DISTINCT room_uuid) as cnt from t_games';
    query(sql1, function (err, rows1, fields) {
        if (err) {
            callback(null);
            return console.error(err.stack);
...
// 通过绑定的agent获取用户id列表
exports.get_userid_by_bind_agent = function (agent, start, row, callback) {
    callback = callback == null ? nop : callback;
    var sql1 = 'SELECT count(*) as cnt FROM t_users where agent="{0}"';
    sql1 = sql1.format(agent);
    query(sql1, function (err, rows1, fields) {
        if (err) {
            callback(null);
            return console.error(err.stack);
...
exports.get_user_data = function(account,callback){
    ...
    var sql = 'SELECT userid,account,name,lv,exp,coins,gems,roomid,last_share_time,invitor FROM t_users WHERE account = "' + account + '"';
    query(sql, function(err, rows, fields) {
        if (err) {
            callback(null);
            // throw err;
